...
def complementarity_residual(x: np.ndarray, problem: Dict[str, Any]) -> float:
    ...
    for i, ub in ubH_finite:
        ...
        upper = abs((-float(G[i])) * (ub - float(H[i])))
    ...
    def build_casadi(t_k: float, delta_k: float, smoothing: str = "product") -> Dict[str, Any]:
        ...
        if ubH_finite:
            ...
            g_parts.append(ca.vcat([
                ca.DM(ub) - H[i] + delta_k for i, ub in ubH_finite
            ]))
            ...
            g_parts.append(ca.vcat([
                (-G[i]) * (ca.DM(ub) - H[i]) - t_k for i, ub in ubH_finite
            ]))
        ...
        # it can correctly recover lambda_G, lambda_H, and lambda_comp for
        # both NCP and box-MCP (gnash*m) problems.
        #
        # Standard NCP layout:
